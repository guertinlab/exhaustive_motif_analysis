...
factor=ZNF143
input_peak_number=300 #MJG change this to $input_peak_number
...
topMockPeaks=${factor}_mock_top${input_peak_number}_summit_100window #MJG you should just call peaks in teh other script
...
cd $dir/Peaks
grep -v "random" ${input_summits} | grep -v "chrUn" | grep -v "chrEBV" | grep -v "chrM" | grep -v "alt" | intersectBed -v -a - -b $blacklist | slopBed -b 50 -i stdin -g $sizes | sort -k1,1 -k2,2n > ${all_peaks}.bed
...
grep -v "random" ${mock_summits} | grep -v "chrUn" | grep -v "chrEBV" | grep -v "chrM" | grep -v "alt" | intersectBed -v -a - -b $blacklist | slopBed -b 50 -i stdin -g $sizes  | sort -k1,1 -k2,2n > ${control}.bed
...
meme -p $ncore -oc $meme_out -nmotifs 1 -minsites $minsites -objfun $objfun -minw 8 -maxw 18 -searchsize 0 -revcomp -dna -bfile $hs1_bkgrnd $topPeaks.fasta
...
# MJG QUESTION HERE, does the p vlaue of a MAST hit change with the number of input sequences?
